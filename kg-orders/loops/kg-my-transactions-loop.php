<?php  
	...
	class KG_My_Transactions_Loop {
		...
		private function get_from_db() {
			$start_from = ( $this->page-1 ) * KG_Config::getPublic('my_transactions_per_page');
			global $wpdb;
			$data = $wpdb->get_results( 
				$wpdb->prepare("
					...
					", $this->user_id,
					   $start_from, 
					   KG_Config::getPublic('my_transactions_per_page')
					 ),
				ARRAY_A 
			); 
			return $data;
		...
		private function count_all_user_transactions() {
			...
			$this->quantity_all_user_transactions = (int) $wpdb->get_var( 
				$wpdb->prepare("SELECT COUNT(id) FROM " . KG_Config::getPublic('table_transactions') . " WHERE user_id = %d", (int) $this->user_id)
			); 
		...
		public function get_page_numbers() {
			 return (int) ceil($this->count_all_user_transactions() / KG_Config::getPublic('my_transactions_per_page'));
		}
