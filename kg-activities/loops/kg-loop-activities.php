<?php  
	...
	class KG_Loop_Activities {
		...
		private function get_from_db() {
			$start_from = ( $this->page-1 ) * KG_Config::getPublic('activities_per_page');
			global $wpdb;
			$data = $wpdb->get_results( 
				$wpdb->prepare("
					SELECT * FROM " . KG_Config::getPublic('table_users_activities') . "
					WHERE user_id = %d 
					ORDER BY date DESC
					LIMIT %d, %d		
					", $this->user_id,
					   $start_from, 
					   KG_Config::getPublic('activities_per_page')
					 ),
				ARRAY_A 
			); 
			return $data;
		...
		private function count_all_activities() {
			if( !empty($this->quantity_all_alerts) ) return $this->quantity_all_alerts;
			global $wpdb;
			$this->quantity_all_alerts = (int) $wpdb->get_var( 
				$wpdb->prepare("SELECT COUNT(id) FROM " . KG_Config::getPublic('table_users_activities') . " WHERE user_id = %d", (int) $this->user_id)
			); 
		...
		public function render($data_from_db) {
			...
			foreach ($data_from_db as $single_item) {
				$out[] = KG_Get_Activity_Object($single_item);
			}
			return $out;
		}
		...
		public function get_page_numbers() {
			 return (int) ceil($this->count_all_activities() / KG_Config::getPublic('activities_per_page'));
		}
		...
		public function get_numbers_found() {
			return $this->count_all_activities();
		}
