<?php  
	...
	class KG_Loop_Box_Stat_Users_Table {
		...
		private function count_all_users() {
			if( !empty($this->quantity_all_users) ) return $this->quantity_all_users;
			global $wpdb;
			$this->quantity_all_users = (int) $wpdb->get_var( 'SELECT COUNT(ID) FROM ' . $wpdb->users); 
			return $this->quantity_all_users;
		}
		...
		public function __construct($params = array()){
			...
			$this->users_per_page = ( !empty($params['users_per_page']) ) ? $params['users_per_page'] : 15;
